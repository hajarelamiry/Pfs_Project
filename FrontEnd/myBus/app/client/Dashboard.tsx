...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (activeFilter === "bus") {
      ...
      const filtered = buses.filter((bus) => bus.stations.some((stationId) => stationIds.includes(stationId)))
      setFilteredBuses(filtered)
    }
  ...
  const handleMapView = () => {
    navigation.navigate("MapView", { buses: filteredBuses })
  }
      ...
      {isLoading ? (
        ...
        <>
          ...
          <FlatList
            ...
            renderItem={({ item }) => <BusItem bus={item} onPress={() => handleBusPress(item)} />}
